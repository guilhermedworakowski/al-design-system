...
var(--al-*). Se um hex ou um px aparecer no input.css, o build para.

Igual ao Select: aqui HA excecao de duracao de motion. O Input e controle,
...
TARGET = os.path.join(HERE, 'input.css')
...
def run():
    if not os.path.exists(TARGET):
        print(f'input.css nao encontrado em {TARGET}')
        return 1
    ...
    usados = set(re.findall(r'var\(\s*(--al-input-[\w-]+)', body))
    ...
    tokens_css = os.path.join(HERE, 'al-input-tokens.css')
    declarados = set()
    if os.path.exists(tokens_css):
        declarados = set(re.findall(r'^\s*(--al-input-[\w-]+)\s*:',
                                    open(tokens_css).read(), flags=re.M))
    orfaos = sorted(declarados - usados)
    inventados = sorted(usados - declarados) if declarados else []

    print('=' * 70)
    print('PORTAO DO CSS DO INPUT')
    print('=' * 70)
    print(f'  arquivo                  : components/input/input.css')
    print(f'  custom properties usadas : {n_vars}  (sendo {len(usados)} da camada do Input)')
    ...
    if inventados:
        print(f'{len(inventados)} CUSTOM PROPERTY DO INPUT QUE NAO EXISTE NA CAMADA DE TOKENS:')
        for t in inventados:
            print('   ', t)
        return 1
