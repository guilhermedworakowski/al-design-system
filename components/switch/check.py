...
var(--al-*). Se um hex ou um px aparecer no switch.css, o build para.

Copia do portao do Radio, com o mesmo segundo portao de token orfao. Ha
excecao de duracao de motion: o Switch e controle, tem hover e foco, e a
...
TARGET = os.path.join(HERE, 'switch.css')
...
def run():
    if not os.path.exists(TARGET):
        print(f'switch.css nao encontrado em {TARGET}')
        return 1
    ...
    usados = set(re.findall(r'var\(\s*(--al-switch-[\w-]+)', body))
    ...
    tokens_css = os.path.join(HERE, 'al-switch-tokens.css')
    declarados = set()
    if os.path.exists(tokens_css):
        declarados = set(re.findall(r'^\s*(--al-switch-[\w-]+)\s*:',
                                    open(tokens_css).read(), flags=re.M))
    orfaos = sorted(declarados - usados)
    inventados = sorted(usados - declarados) if declarados else []

    print('=' * 70)
    print('PORTAO DO CSS DO SWITCH')
    print('=' * 70)
    print(f'  arquivo                  : components/switch/switch.css')
    print(f'  custom properties usadas : {n_vars}  (sendo {len(usados)} da camada do Switch)')
    ...
    if inventados:
        print(f'{len(inventados)} CUSTOM PROPERTY DO SWITCH QUE NAO EXISTE NA CAMADA DE TOKENS:')
        for t in inventados:
            print('   ', t)
        return 1
