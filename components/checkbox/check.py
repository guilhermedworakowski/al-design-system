...
var(--al-*). Se um hex ou um px aparecer no checkbox.css, o build para.

Copia do portao do Select, com o mesmo segundo portao de token orfao. Ha
excecao de duracao de motion: o Checkbox e controle, tem hover e foco, e a
...
TARGET = os.path.join(HERE, 'checkbox.css')
...
def run():
    if not os.path.exists(TARGET):
        print(f'checkbox.css nao encontrado em {TARGET}')
        return 1
    ...
    usados = set(re.findall(r'var\(\s*(--al-checkbox-[\w-]+)', body))
    ...
    tokens_css = os.path.join(HERE, 'al-checkbox-tokens.css')
    declarados = set()
    if os.path.exists(tokens_css):
        declarados = set(re.findall(r'^\s*(--al-checkbox-[\w-]+)\s*:',
                                    open(tokens_css).read(), flags=re.M))
    orfaos = sorted(declarados - usados)
    inventados = sorted(usados - declarados) if declarados else []

    print('=' * 70)
    print('PORTAO DO CSS DO CHECKBOX')
    print('=' * 70)
    print(f'  arquivo                  : components/checkbox/checkbox.css')
    print(f'  custom properties usadas : {n_vars}  (sendo {len(usados)} da camada do Checkbox)')
    ...
    if inventados:
        print(f'{len(inventados)} CUSTOM PROPERTY DO CHECKBOX QUE NAO EXISTE NA CAMADA DE TOKENS:')
        for t in inventados:
            print('   ', t)
        return 1
