...
var(--al-*). Se um hex ou um px aparecer no select.css, o build para.

Igual ao Button e ao Icon Button, e diferente do Tag e do Avatar: aqui HA
excecao de duracao de motion. O Select e controle, tem hover e foco, e por isso
tem transicao - e a Foundation ainda nao tem escala de motion para os 120ms
...
TARGET = os.path.join(HERE, 'select.css')
...
def run():
    if not os.path.exists(TARGET):
        print(f'select.css nao encontrado em {TARGET}')
        return 1
    ...
    usados = set(re.findall(r'var\(\s*(--al-select-[\w-]+)', body))

    # Segundo portao, proprio do Select: token declarado e nunca usado e token
    # que so parece existir. Compara o que o tokens.py emitiu com o que o CSS
    # consome, pelo arquivo de tokens gerado ao lado.
    tokens_css = os.path.join(HERE, 'al-select-tokens.css')
    declarados = set()
    if os.path.exists(tokens_css):
        declarados = set(re.findall(r'^\s*(--al-select-[\w-]+)\s*:',
                                    open(tokens_css).read(), flags=re.M))
    orfaos = sorted(declarados - usados)
    inventados = sorted(usados - declarados) if declarados else []

    print('=' * 70)
    print('PORTAO DO CSS DO SELECT')
    print('=' * 70)
    print(f'  arquivo                  : components/select/select.css')
    print(f'  custom properties usadas : {n_vars}  (sendo {len(usados)} da camada do Select)')
    ...
    if inventados:
        print(f'{len(inventados)} CUSTOM PROPERTY DO SELECT QUE NAO EXISTE NA CAMADA DE TOKENS:')
        for t in inventados:
            print('   ', t)
        return 1
